...
def configure_password(path: str | Path, *, prompt: Callable[[str], str] = getpass.getpass) -> None:
    password = prompt("New Igiris password: ")
    if len(password) < MIN_PASSWORD_LENGTH:
        raise ValueError(f"Password must be at least {MIN_PASSWORD_LENGTH} characters")
    confirmation = prompt("Confirm Igiris password: ")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Set the local Igiris unlock password")
    parser.add_argument(
        "--file",
    ...
    print(f"Igiris password verifier updated: {args.file}")
